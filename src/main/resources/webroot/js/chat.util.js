...
//			$("#chatbox").show();
		...
		//添加好友
		add:function(){
			var id = $("#addfriend").val();
			if(id==null){
				alert("好友名不能为空");
				return;
			}
			var msg = "[ADD][" + new Date().getTime() +"][" + CHAT.sender + "]["+id+"] - "+"添加好友";
		...
		sendText:function(){
			...
			if(CHAT.socket.readyState == WebSocket.OPEN){
				...
				var msg = "[CHAT][" + new Date().getTime() +"][" + CHAT.sender + "]["+CHAT.receiver+"] - " + input.html().replace(/\n/ig,"<br/>");
				CHAT.socket.send(msg);
				input.empty();
				input.focus();
			}else{
		...
		init :function(sender){
			...
			var appendToPanel = function(msg){
				...
				if(cmd=="REGISTER"){
					...
					if(content=="0"){
						...
						var friends = content.split(",");
						var root = $("#friends");
						var str ="";
						for(var i=0;i<friends.length;i++){
							str +='<input class="button" type="button" id="friend" onclick="CHAT.selectFriend(this)" value='+friends[i]+">"
						}
						root.append(str)
					}
				}else if(cmd == "SYSTEM"){
					//添加一个系统时间标签
					CHAT.addSystemTip('<span class="time-label">' + content + '</span>');
				}else if(cmd == "CHAT"){
					//解析消息的发送时间
			...
			if(window.WebSocket){
				...
				CHAT.socket.onopen = function(e){
					if(CHAT.isLogin){
						CHAT.socket.send("[LOGIN][" + new Date().getTime() + "][" + CHAT.sender + "]" + " - " + $("#pwd").val());
					}else{
						CHAT.socket.send("[REGISTER][" + new Date().getTime() + "][" + CHAT.sender + "]" + " - " + $("#pwd").val())
					}
				}
